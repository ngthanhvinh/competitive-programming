...
void dfs(int u) {
	...
	for (int x = 1; x <= n; x++) f[u][x] += x; 
	long long mn1 = 1e18, Pos1 = 0;
	
	for (int x = 1; x <= n; x++) if (mn1 > f[u][x]) { mn1 = f[u][x]; Pos1 = x; }
	long long mn2 = 1e18, Pos2 = 0;
	for (int x = 1; x <= n; x++) if (x != Pos1 && mn2 > f[u][x]) { mn2 = f[u][x]; Pos2 = x; }
...
int main() {
	...
	for (int i = 1; i <= n; i++) if (mn > f[1][i]) { mn = f[1][i]; color[1] = i; }
	trace(1, color[1]);
	printf("%lld\n", mn);
